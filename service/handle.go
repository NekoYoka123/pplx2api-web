...
func ChatCompletionsHandler(c *gin.Context) {
	...
	if requestModel == "" {
		requestModel = config.ConfigInstance.DefaultModel
	}
	if config.ConfigInstance.ForceModel != "" {
		requestModel = config.ConfigInstance.ForceModel
	}
	openSearch := false
	baseModel := requestModel
	if strings.HasSuffix(baseModel, "-search") {
		openSearch = true
		baseModel = strings.TrimSuffix(baseModel, "-search")
	}
	modelPreference := config.ModelMapGet(baseModel, baseModel) // 获取模型名称
	...
	for i := 0; i < config.ConfigInstance.RetryCount; i++ {
		...
		logger.Info(fmt.Sprintf("Using session for model %s (preference: %s): %s", requestModel, modelPreference, session.SessionKey))
		if err != nil {
		...
		pplxClient = core.NewClient(session.SessionKey, config.ConfigInstance.Proxy, modelPreference, requestModel, openSearch)
