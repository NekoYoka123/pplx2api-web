...
func (su *SessionUpdater) loadSessionsFromFile() {
	...
	if _, err := os.Stat(su.configPath); os.IsNotExist(err) {
		log.Println("No config file found, will create on first update")
		return
	}

	fileConfig, err := config.LoadConfigFile(su.configPath)
	if err != nil {
		log.Printf("Failed to read config file: %v", err)
		return
	}
	sessions := make([]config.SessionInfo, len(fileConfig.Sessions))
	...
	log.Printf("Loaded %d sessions from config file", len(sessions))
}

// saveSessionsToFile saves the current sessions to the config file
func (su *SessionUpdater) saveSessionsToFile() error {
	snapshot := config.SnapshotConfigFile()
	if err := config.SaveConfigFile(su.configPath, snapshot); err != nil {
		return err
	}
	log.Printf("Saved %d sessions to config.json file", len(snapshot.Sessions))
	return nil
}
...
func (su *SessionUpdater) updateAllSessions() {
	...
	for i, session := range sessionsCopy {
		...
		go func(index int, origSession config.SessionInfo) {
			...
			client := core.NewClient(origSession.SessionKey, proxy, "claude-3-opus-20240229", "claude-3-opus-20240229", false)
