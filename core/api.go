...
func overlapSuffixPrefix(base string, next string) int {
	...
	nextLimit := len(next)
	if nextLimit > maxCheck {
		nextLimit = maxCheck
	}
	minOverlap := 3
	for overlap := nextLimit; overlap >= minOverlap; overlap-- {
		if strings.HasSuffix(base, next[:overlap]) {
			return overlap
		}
	}
	return 0
...
func mergeChunkWithOverlap(base string, chunk string) string {
	...
	if strings.HasPrefix(chunk, base) {
		return chunk
	}
	...
	prefixLimit := len(chunk)
	if prefixLimit > 200 {
		prefixLimit = 200
	}
	searchChunk := chunk[:prefixLimit]
	...
	for overlap := maxCheck; overlap >= minOverlap; overlap-- {
		suffix := base[len(base)-overlap:]
		pos := strings.Index(searchChunk, suffix)
		if pos >= 0 {
			return base + chunk[pos+overlap:]
		}
	}
	return base + chunk
}
...
func combineChunks(chunks []string) string {
	...
	for _, chunk := range chunks {
		...
		combined = mergeChunkWithOverlap(combined, chunk)
	}
	return combined
}

// NewClient creates a new Perplexity API client
func NewClient(sessionToken string, proxy string, model string, requestModel string, openSerch bool) *Client {
	...
	if requestModel == "" {
		requestModel = config.ModelReverseMapGet(model, model)
	}
	c := &Client{
...
func (c *Client) HandleResponse(body io.ReadCloser, stream bool, gc *gin.Context) error {
	...
	for scanner.Scan() {
		...
		if response.DisplayModel != "" {
			displayModel = config.ModelReverseMapGet(response.DisplayModel, response.DisplayModel)
		...
		if response.Status == "COMPLETED" {
			...
			if !config.ConfigInstance.IgnoreModelMonitoring && displayModel != "" {
				requestedModel := config.ModelReverseMapGet(c.Model, c.Model)
				if displayModel != requestedModel {
					res_text := "\n\n---\n"
		...
		for _, block := range response.Blocks {
			if block.MarkdownBlock != nil && len(block.MarkdownBlock.Chunks) > 0 {
				markdownText := combineChunks(block.MarkdownBlock.Chunks)
				if markdownText == "" {
					continue
				}
	...
	if !stream {
		if finalThinkingText != "" {
			full_text += "<think>" + finalThinkingText + "</think>\n\n"
		}
		full_text += finalMarkdownText
		full_text += finalImageText
		full_text += finalWebText
